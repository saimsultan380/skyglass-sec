...
const trustItems = [
  { label: "Verified Customers", icon: BadgeCheck },
  { label: "Device Context", icon: MonitorSmartphone },
  { label: "Balanced Feedback", icon: Scale },
  { label: "Support Experience", icon: Headphones },
];
    ...
    <>
      <div className="w-full max-w-none" data-no-reveal>
        <MaskReveal
          ...
          parts={[
            { text: "Sky Glass IPTV Reviews UK –" },
            {
              text: "Customer Experiences, Devices & Support",
...
function HeroCTAs() {
  return (
    <div className="flex w-full max-w-full min-w-0 flex-col items-stretch gap-2 sm:gap-3 lg:flex-col xl:flex-row xl:items-center">
      <Link
        href={`${ROUTES.contact}?enquiry=review`}
      ...
      >
        ...
        >
          <PenLine className="mr-1.5 h-3.5 w-3.5 shrink-0 stroke-[2.5] sm:mr-2 sm:h-4 sm:w-4" />
          <span className="hidden xl:inline">Submit an Honest Review</span>
          <span className="inline xl:hidden">Submit a Review</span>
        </Button>
      </Link>
