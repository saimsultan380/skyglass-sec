    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start w-full">
          {/* Device picker — left, sticky, scrollable (not full length) */}
          <div
            data-no-reveal
            className="lg:col-span-4 flex flex-col overflow-hidden rounded-[12px] bg-white p-5 sm:p-6 lg:sticky lg:top-24 lg:max-h-[min(70vh,36rem)]"
          >
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto overscroll-contain pr-1 [-ms-overflow-style:auto] [scrollbar-width:thin]">
              {deviceGuidesList.map((device) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex shrink-0 items-center justify-between text-left px-4 py-3 rounded-[12px] border-0 transition-all duration-200 focus:outline-none ${
                      isActive
                        ? "bg-pink-50/60 text-[#E91E8C]"
          ...
          {/* Guide content — right on desktop */}
          <div
            id={activeGuide.id}
            ref={contentRef}
            data-no-reveal
            className="lg:col-span-8 rounded-[12px] bg-white p-6 sm:p-8 flex flex-col justify-between min-h-[600px] relative scroll-mt-24"
          >
            <div>
              ...
              {activeGuide.showDownloaderCode && (
                <div className="inline-flex items-center gap-3 p-3 rounded-[12px] bg-pink-50/50 mb-6 select-none">
              ...
              <ol className="space-y-6 relative border-l-2 border-white/30 pl-6 ml-3">
                {activeGuide.steps.map((step, idx) => (
                  <li key={step.title} className="relative">
                    <span className="absolute -left-[37px] top-0.5 flex h-6 w-6 items-center justify-center rounded-full bg-pink-50/80 text-[#E91E8C] font-bold text-xs">
                      {idx + 1}
                    </span>
              ...
              <div className="mt-8">
                <Link href={ROUTES.contact} className="w-full sm:w-auto inline-block">
                  <Button
                    variant="outline"
                    size="lg"
                    className="w-full sm:w-auto rounded-[12px] border-0 px-5 sm:px-6 py-3.5 text-xs sm:text-sm font-semibold"
                  >
            ...
            {activeGuide.notes && activeGuide.notes.length > 0 && (
              <div className="border-t border-white/25 pt-5 mt-8 space-y-2">
